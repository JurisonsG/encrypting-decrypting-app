...
def des_block_encrypt(block, subkeys):
    ...
    ip = permute(block_int, IP, 64)
    L, R = (ip >> 32) & 0xFFFFFFFF, ip & 0xFFFFFFFF
    for k in subkeys:
        L, R = R, L ^ feistel(R, k)
    preout = (R << 32) | L
    final = permute(preout, FP, 64)
...
def pkcs7_unpad(data, block_size=8):
    pad_len = data[-1]
    return data[:-pad_len]
...
def triple_des_encrypt(plaintext, key1, key2, key3):
    ...
    k3 = int.from_bytes(key3, 'big')
    sub1 = generate_subkeys(k1)
    sub2 = generate_subkeys(k2)
    sub3 = generate_subkeys(k3)
    data = pkcs7_pad(plaintext)
    out = bytearray()
    for i in range(0, len(data), 8):
        b = data[i:i+8]
        b = des_block_encrypt(b, sub1)
        b = des_block_decrypt(b, sub2)
        b = des_block_encrypt(b, sub3)
        out.extend(b)
    return bytes(out)
...
def triple_des_decrypt(ciphertext, key1, key2, key3):
    ...
    for i in range(0, len(ciphertext), 8):
        b = ciphertext[i:i+8]
        b = des_block_decrypt(b, sub3)
        b = des_block_encrypt(b, sub2)
        b = des_block_decrypt(b, sub1)
        out.extend(b)
